...
<header id='top' class='relative screen'>
  ...
  >
    <HeaderSplash
      class="{factor.x >= 1 ? 'scale-100' : ''} absolute right-0 top-0 mx-auto transform-translate-x-[var(--t-x,0%)] transform h-lvh media-squarish:right-40% media-squarish:transform-translate-x-[calc(50%+var(--t-x,0%))]"
      {scale}
      {splash_rel}
    />
    <div
